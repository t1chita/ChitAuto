...
final class ReleaseDateCell: UITableViewCell {
    //MARK: - Properties
    ...
    private let releaseDate: GeneralLabel = {
    ...
    //MARK: - Configure
    func configure(withReleaseDate releaseDate: String) {
    ...
    private func setReleaseDate() {
        rectangleView.addSubview(releaseDate)
        
        NSLayoutConstraint.activate([
            releaseDate.centerYAnchor.constraint(equalTo: rectangleView.centerYAnchor),
            releaseDate.heightAnchor.constraint(equalToConstant: 17),
            releaseDate.trailingAnchor.constraint(equalTo: rectangleView.trailingAnchor, constant: -34),
        ])
    }
}
